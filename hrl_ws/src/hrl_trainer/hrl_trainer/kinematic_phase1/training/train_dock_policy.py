"""Train the Phase 1B dock policy."""
...
def _require_training_dependencies() -> None:
    try:
        ...
        raise RuntimeError("Dock training requires gymnasium and stable-baselines3 in the active environment.") from exc


def _load_config(explicit_path: str | None) -> dict:
    cfg = deep_merge(load_yaml_file(dock_default_config_path()), load_yaml_file(ppo_default_config_path()))
    if explicit_path:
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Train the Phase 1B dock policy.")
    parser.add_argument("--config")
    parser.add_argument("--run-id", default="dock_policy")
...
def main() -> None:  # pragma: no cover - CLI integration
    ...
    artifact_root = Path(args.artifact_root) if args.artifact_root else default_artifact_root("phase1b_dock", args.run_id)
    ...
    if args.resume_from:
        ...
        print(f"Resuming dock policy from {args.resume_from}")
    else:
