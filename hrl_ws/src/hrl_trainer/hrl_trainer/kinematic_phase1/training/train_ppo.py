"""Train a Phase 1 PPO baseline with Stable-Baselines3."""
...
def _require_training_dependencies() -> None:
    try:
        ...
        raise RuntimeError(
            "Phase 1 PPO training requires gymnasium and stable-baselines3 in the active environment."
        ) from exc


def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Train a Phase 1 PPO baseline.")
    parser.add_argument("--config")
    parser.add_argument("--run-id", default="ppo_baseline")
...
def main() -> None:  # pragma: no cover - CLI integration
    ...
    cfg = load_training_config("ppo", args.config)
    ...
    artifact_root = Path(args.artifact_root) if args.artifact_root else default_artifact_root("ppo", args.run_id)
